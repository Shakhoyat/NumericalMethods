...
void jacobiMethod() {
  ...
  if (!is_diag_dominant(A)) {
    cout << "Warning: The matrix is not diagonally dominant. Attempting to "
            "make it diagonally dominant..."
         << endl;
    if (!make_diag_dominant(A, b)) {
      cout << "Error: Unable to make the matrix diagonally dominant. The "
  ...
  for (int iter = 0; iter < highest_iter && !converged; iter++) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (fabs(A[i][i]) < 1e-9) {
        cout << "\nError: Zero or near-zero diagonal element at row " << i + 1
             << "." << endl;
        return;
      }
  ...
  if (!converged) {
    cout << "Warning: Solution did not converge within " << highest_iter
         << " iterations." << endl;
  }
...
void gaussSeidelMethod() {
  ...
  if (!is_diag_dominant(A)) {
    ...
    if (!make_diag_dominant(A, b)) {
      cout << "Error: Unable to make the matrix diagonally dominant. The "
              "Gauss-Seidel method may not converge."
           << endl;
      return;
    }
  }
  ...
  for (int iter = 0; iter < highest_iter && !converged; iter++) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (fabs(A[i][i]) < 1e-9) {
        cout
            << "\nError: Zero or near-zero diagonal element encountered at row "
            << i + 1 << "." << endl;
        return;
      }
  ...
  if (!converged) {
    cout << "\nSolution did not converge within " << highest_iter
         << " iterations." << endl;
  }
...
void gaussElimination() {
  ...
  for (int i = 0; i < n; i++) {
    for (int j = i + 1; j < n; j++) {
      ...
      cout << "Eliminating variable x" << i + 1 << " from row " << j + 1
           << " using factor " << factor << endl;
      for (int k = i; k < n; k++)
        A[j][k] -= factor * A[i][k];
      b[j] -= factor * b[i];

      cout << "Updated Augmented Matrix after elimination from row " << j + 1
           << ":" << endl;
      printMatrix(A, b);
    }
  }
...
void LU_Decomposition(vector<vector<double>> &A, vector<vector<double>> &L,
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (fabs(A[i][i]) < 1e-9) {
      ...
      if (!swapped) {
        cout << "Error: Matrix is singular; LU Factorization cannot be "
                "performed."
             << endl;
        return;
      }
