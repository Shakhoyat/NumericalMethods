...
void ApplyNewtonRaphson(function<double(double)> eqn,
                        function<double(double)> derivative, double x0);

// Evaluating the polynomial at a giveNon_lin_mainn point
...
int main() {
  ...
  while (true) {
    ...
    cout
        << "Enter the polynomial coefficients from highest to lowest degree:\n";
    for (int i = 0; i <= poly_degree; i++) {
...
void ApplyBisection(function<double(double)> eqn, double start, double end) {
  ...
  cout << "Approximate root after " << kMaxIterations
       << " iterations: " << midpoint << endl;
}

// False Position Method Implementation
void ApplyFalsePosition(function<double(double)> eqn, double start,
                        double end) {
  if (eqn(start) * eqn(end) >= 0) {
    cout << "No root in this interval.\n";
    return;
...
void ApplySecant(function<double(double)> eqn, double x0, double x1) {
  ...
  cout << "Approximate root after " << kMaxIterations << " iterations: " << x2
       << endl;
}
...
void ApplyNewtonRaphson(function<double(double)> eqn,
  ...
  cout << "Approximate root after " << kMaxIterations << " iterations: " << x1
       << endl;
}
